"""
ORIGINAL VERSION - Before new PC changes
This is a backup of the working version that uses combine_text_fields_weighted with all ~44 fields.
...
def main():
    print("=" * 70)
    print("Generate Embeddings from PostgreSQL (ORIGINAL VERSION)")
    print("=" * 70)
    print()
    ...
    try:
        ...
        # Store in database
        print("Step 9: Storing embeddings in database...")
        ...
        for i in tqdm(range(0, len(records), batch_size), desc="Storing embeddings", total=total_batches):
        ...
        print(f"✓ Stored {len(records):,} embeddings in database")
        print()
        ...
        print(f"✓ Final count: {final_count:,} embeddings")
        print()
        ...
        print("=" * 70)
        print()
        print(f"Generated {final_count:,} embeddings from {len(requests):,} requests")
        print(f"Average chunks per request: {final_count / len(requests):.2f}")
        print()
